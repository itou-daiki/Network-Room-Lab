...
export const ROLE_READING_GUIDES: Record<CoreRoleId, RoleReadingGuideItem[]> = {
  CLIENT_PC: [
    ...
    { target: "PCの出口：192.168.10.1", reading: "192.168.10.1は、PCと同じ校内LANにつながるルータのLAN側IPアドレスです。PCは、別のネットワークへ送るデータを最初にこのルータへ渡します。この出口をデフォルトゲートウェイと呼びます。", check: "まずipconfigで出口の設定を見ます。次にping 192.168.10.1という確認コマンドで、PCから出口のルータまで返事が来るかを確かめます。" },
...
export const ROLE_PRACTICES: RolePracticeDefinition[] = [
  {
    role: "CLIENT_PC",
    mission: "Webサーバへ送る「このページをください」という要求の最初の渡し先を決め、そのルータへ渡すためのMACアドレスを調べる",
    beginnerStory: "あなたは、Webページを見たい人が使っているPCです。ページを見るためには、まずPCからWebサーバへ「このページをください」というお願いのデータを送ります。この時点でPCが送るのは、Webページそのものではありません。Webサーバはお願いを受け取った後、ページのデータをPCへ返します。Webサーバが別のネットワークにある場合、PCはお願いを直接Webサーバへ渡さず、最初に出口のルータへ渡します。",
    everydayExample: "遠くの店から商品を受け取りたいときは、先に注文書を送ります。注文書は、まず近くの郵便局へ渡され、遠くの店まで運ばれます。PCも同じように、最初に渡す相手を近くの郵便局に当たるルータへ決めます。さらに、同じ地域内でその郵便局へ確実に渡すための識別番号に当たるMACアドレスを調べます。",
    ...
    observations: [
      ...
      { label: "外部への出口（デフォルトゲートウェイ）", value: "192.168.10.1", meaning: "PCと同じLANにつながるルータのLAN側IPアドレスです。別のネットワークへ送るとき、PCは最初にこのルータへデータを渡します。" },
      { label: "ARPが動く場所", value: "このPCの中（OSのネットワーク機能）", meaning: "ARP専用の機器やサーバへ接続するのではありません。このPC内のARP機能が問い合わせを作り、結果をPC内のARPキャッシュへ保存します。" },
      { label: "IPとMACの対応を覚えた表（ARPキャッシュ）", value: "192.168.10.1 → MACアドレスは未登録", meaning: "PCは出口のIPアドレスを知っていますが、同じLAN内で実際にルータへ渡すための宛先MACアドレスをまだ知りません。" },
    ],
    decisionHint: "自分と宛先のIPはネットワークの範囲が異なります。PCはまず、同じLANにいる出口のルータへ渡します。Wi-FiやEthernetでそのルータへ届けるには、ルータのMACアドレスが必要です。",
    question: "別ネットワークのWebサーバへ要求を送るために、最初にデフォルトゲートウェイのルータへ渡します。PCはまだルータのMACアドレスを知りません。次に行う操作はどれですか？",
    choices: [
      ...
      { id: "pc-arp-gateway", label: "このPC内のARP機能を使い、192.168.10.1に対応するルータのMACアドレスを調べる", correct: true, feedback: "正解です。ARP専用の機器へ質問するのではありません。このPC内のARP機能が、同じLAN内へ問い合わせを送り、192.168.10.1を使っているルータ内のARP機能がMACアドレスを答えます。" },
      { id: "pc-change-ip", label: "自分のIPアドレスをWebサーバと同じ値へ変更する", correct: false, feedback: "同じIPアドレスを使うと重複します。異なるネットワークへはルータを経由します。" },
    ],
    successTitle: "出口のルータへ渡す準備ができました",
    successOutput: [
      "PCの判断：Webサーバは別のネットワークにあるため、最初の渡し先をデフォルトゲートウェイ192.168.10.1にする",
      "PC内のARP機能が送る問い合わせ：192.168.10.1を使っている機器へ。あなたのMACアドレスを教えてください",
      "ルータ内のARP機能が返す応答：192.168.10.1を使っているのは私です。私のMACアドレスは02-00-00-00-10-01です",
      "PCが作るフレーム：宛先MACを02-00-00-00-10-01にして、Webサーバへの要求を出口のルータへ渡す",
    ],
    successMeanings: [
      "192.168.10.1は、PCへあらかじめ設定されているルータのLAN側IPアドレスです。ARPで出口を見つけたのではなく、PCが最初に渡す相手として選びました。郵便に例えると、遠い宛先への郵便物を最初に近くの郵便局へ渡すと決めた段階です。",
      "ARP専用の機器やサーバはありません。PCのOSに組み込まれたARP機能が、この問い合わせを同じLAN内の全機器へ送ります。",
      "192.168.10.1はルータのLAN側IPアドレスです。そのIPアドレスを使うルータ内のARP機能が、自分のMACアドレスを答えました。PCは答えをPC内のARPキャッシュへ一時保存します。",
      "Webサーバへ送る「このページをください」というIPパケットの最終宛先はWebサーバのままです。PCは、そのパケットをルータのMACアドレス宛てのフレームに入れ、まず出口のルータへ渡します。",
    ],

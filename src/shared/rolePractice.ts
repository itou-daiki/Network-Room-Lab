...
export const ROLE_READING_GUIDES: Record<CoreRoleId, RoleReadingGuideItem[]> = {
  ...
  WEB_SERVER: [
    { target: "Webサーバの住所と受付：203.0.113.80:443", reading: "コロンの左がWebサーバのIPアドレス、右の443が安全なWeb通信を受け付ける窓口番号です。", check: "curl https://class.yamanashi.exampleという確認コマンドで、接続先・証明書・教材ページの応答を順番に確認します。" },
...
export const ROLE_PRACTICES: RolePracticeDefinition[] = [
  {
    ...
    observations: [
      ...
      { label: "Webサーバの住所（宛先IP）", value: "203.0.113.80", meaning: "これからデータを届けたいWebサーバの住所です。PCの住所と範囲が違うため、直接は渡せません。" },
    ...
    question: "別ネットワークのWebサーバへデータを送り始めるために、このPCが最初に行う操作はどれですか？",
    choices: [
  ...
  },
  {
    ...
    situation: "PCから「class.yamanashi.exampleのIPv4アドレスを教えて」という質問が届きました。答えはDNSサーバに登録されています。",
    observationTitle: "PCからの質問とDNSサーバの登録内容",
    observationPurpose: "PCが質問したWebサイト名を確認し、それに対応する正しいIPアドレスを答えるためです。",
    observations: [
      { label: "PCが入力したWebサイト名（問い合わせ名）", value: "class.yamanashi.example", meaning: "PCがIPアドレスを知りたいWebサイトの名前です。" },
    ...
    successOutput: ["質問の処理結果：成功（NOERROR）", "DNSの回答：class.yamanashi.example → 203.0.113.80（300秒保存可）", "PCへ返す答え：1件"],
  ...
  },
  {
    ...
    situation: "PCとの通信路と暗号化の準備が終わり、「/lessonのページをください」という要求が届きました。",
    ...
    observations: [
      ...
      { label: "求められたページの場所（パス）", value: "/lesson", meaning: "Webサーバ内で、PCが求めている教材ページの場所です。" },
    ],
    decisionHint: "通信路と暗号化の準備は完了しています。GETは「ページをください」なので、指定された/lessonを成功応答と一緒に返します。",
    question: "PCが求めた教材ページを表示できるようにするために、Webサーバはどの答えを返しますか？",
    choices: [
    ...
    successOutput: ["Webサーバの回答：200 OK（要求を処理できた）", "返す内容の種類：HTML / 文字コード UTF-8", "通信の保護：TLSで暗号化", "返した内容：教材ページの本文"],

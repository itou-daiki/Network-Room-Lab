...
export const NETWORK_GLOSSARY: NetworkTerm[] = [
  ...
  { id: "arp-cache", label: "ARPキャッシュ", short: "ARPで調べたIPとMACの対応を一時的に覚える表。", detail: "毎回問い合わせなくてよいよう、同じLAN内の機器についてIPアドレスとMACアドレスの組を保存します。", example: "arp -aで、192.168.10.1と02-00-00-00-10-01の対応を確認します。", category: "通信" },
  ...
  { id: "http", label: "HTTP", reading: "エイチティーティーピー", short: "Webページを要求し、応答を返すための決まり。", detail: "クライアントがメソッドとパスを指定し、Webサーバがステータスコードと内容を返します。", example: "GET /lesson に対して 200 OKとHTMLを返します。", category: "通信" },
  { id: "http-method", label: "HTTPメソッド", short: "Webサーバへ何をしてほしいかを示す操作名。", detail: "GETは取得、POSTはデータ送信など、要求の目的を先頭に記します。", example: "GET /lesson は、教材ページを取得する要求です。", category: "通信" },
  { id: "status-code", label: "HTTPステータスコード", short: "Webサーバによる処理結果を3桁の数で示したもの。", detail: "200番台は成功、400番台は要求側、500番台はサーバ側の問題を表します。", example: "200 OKは要求を正常に処理できた意味です。", category: "通信" },
  ...
  { id: "l2-switch", label: "L2スイッチ", short: "MACアドレスを見て、同じLAN内のフレームを転送する機器。", detail: "送信元MACを受信ポートとともに学習し、宛先MACに対応するポートを選びます。", example: "ルータのMACがport 2にあれば、フレームをport 2だけへ送ります。", category: "機器・接続" },
  { id: "mac-table", label: "MACアドレス表", short: "MACアドレスと接続ポートの対応を記録した表。", detail: "L2スイッチはフレームの送信元から表を学習し、宛先を転送するときに参照します。", example: "02-00-00-00-10-01 → port 2 のように読みます。", category: "通信" },
  { id: "dns-record", label: "DNSレコード", short: "ドメイン名に関する情報を登録した項目。", detail: "種類ごとに内容が異なり、Aレコードは名前に対応するIPv4アドレスを示します。", example: "class.yamanashi.example. 300 IN A 203.0.113.80", category: "通信" },

...
export const NETWORK_GLOSSARY: NetworkTerm[] = [
  ...
  { id: "gateway", label: "デフォルトゲートウェイ", short: "別のネットワークへ送るとき、PCが最初にデータを渡すルータです。", detail: "PCは宛先が別のネットワークにあると判断すると、データを直接その宛先へ渡さず、同じLANにいるルータへ渡します。デフォルトゲートウェイとしてPCへ設定される値は、そのルータのLAN側IPアドレスです。郵便に例えると、遠い宛先への郵便物を最初に渡す近くの郵便局に当たります。", example: "この実習では、ルータのLAN側IPアドレス192.168.10.1がPCのデフォルトゲートウェイです。", category: "アドレス" },
  ...
  { id: "arp", label: "ARP", reading: "アープ", fullName: "Address Resolution Protocol（アドレス・リゾリューション・プロトコル／アドレス解決プロトコル）", short: "PCやルータ本体の中で動き、同じLAN内のIPv4アドレスからMACアドレスを調べるネットワーク機能です。", detail: "ARP専用の機器やARPサーバがあるのではありません。ARPはPCやルータ本体の中にあるネットワーク機能です。PCではブラウザやアプリではなく、OSのネットワーク機能の一部として動きます。ルータでは、ルータ内部のネットワーク機能として動きます。Addressは「住所」、Resolutionは「対応する答えを見つけること」、Protocolは「通信の約束」を表します。PC内のARP機能は、同じLAN内の全機器へ「このIPv4アドレスを使っている機器は、あなたですか。MACアドレスを教えてください」と問い合わせます。該当する機器内のARP機能だけが自分のMACアドレスを返し、PCはその組をPC内のARPキャッシュへ一時保存します。Webサーバが別ネットワークにある場合、PCがARPで調べるのはWebサーバではなく、最初の出口であるデフォルトゲートウェイです。画面に出る問い合わせ文はコマンドではなく、機器同士が交換する通信内容です。", example: "PC内のARP機能は、デフォルトゲートウェイ192.168.10.1へデータを渡すため、同じLAN内のルータのMACアドレス02-00-00-00-10-01を調べます。", category: "通信" },
  { id: "arp-cache", label: "ARPキャッシュ", short: "ARPで調べたIPアドレスとMACアドレスの組を、PCやルータの中で一時的に覚える表です。", detail: "ARPキャッシュはネットワーク上にある別のサーバではなく、PCやルータがそれぞれ自分のメモリ内に持つ表です。同じ機器へ送るたびにARP問い合わせをしなくてよいよう、調べた組を一定時間保存します。", example: "PC内の表を実際に見るときは、arp -aという確認コマンドを使います。192.168.10.1と02-00-00-00-10-01が同じ行にあれば、PCが出口のルータのMACアドレスを覚えています。", category: "通信" },
  { id: "broadcast", label: "ブロードキャスト", short: "同じLAN内の全機器へ、まとめて問い合わせを届ける送り方です。", detail: "ARPを使って調べ始める時点では、相手のMACアドレスがまだ分かりません。そこで宛先MACアドレスをFF-FF-FF-FF-FF-FFとして同じLAN全体へ送り、該当するIPアドレスを持つ機器だけが応答します。ルータを越えてインターネット全体へ広がるものではありません。", example: "PCがARPを使い、「192.168.10.1を使っている機器はMACアドレスを教えてください」と校内LAN全体へ送る問い合わせです。", category: "通信" },

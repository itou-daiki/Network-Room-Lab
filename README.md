...
情報ネットワークを「役割分担して動かす」協働型Webアプリケーションです。教員が授業用の部屋を作り、学習者が PC・無線AP・L2スイッチ・ルータ・DNSサーバ・Webサーバを担当して、Webページが表示されるまでの通信と障害診断を体験できます。
...
部屋ごとに1つの Durable Object を使用し、参加者、状態、イベント、振り返り、スナップショットをSQLiteへ保存します。WebSocket接続が休止している間も接続情報を保持できるため、教室内のリアルタイム更新を低コストで継続できます。

...
「文部科学省の学習指導要領ページを見る」という大きな目的に向かって、情報ネットワークを役割ごとに動かす体験型Webアプリケーションです。一人なら6つの機器を順番に、授業では仲間と役割分担して、Webページが表示されるまでの通信と原因調査を体験できます。
...
- 1回の操作で最大10部屋を作り、各班を2〜6人に設定できる教員用一括作成
- 作成した全部屋のコード一覧・一括コピー・教員として開く操作を同じ画面に集約
- 2〜5人班では、担当者がいない機器の通信段階を班全員で操作できる少人数対応
- 部屋コード不要で全6役を順番に体験できる、自己ペースのひとり学習モード
- 学習状態に合わせて「次にやる1操作」「終わった後」「操作場所」を示し続ける学習コーチ
- 主要課題を先に表示し、全体図・履歴・用語集は必要なときだけ開ける初学者向け画面構成
- PC、無線AP、L2スイッチ、ルータ、DNS、Webサーバを「目的→情報→操作→結果→説明」の1画面1行動で順番に体験する初学者向け役割学習
- 「DNSサーバへIPアドレスを聞く往復」と「Webサーバへ学習指導要領ページを取りに行く往復」を、行き・帰り・現在地付きで示す詳細な経路図
- 画面情報ごとのやさしい意味、身近な例、任意ヒント、説明の書き出し支援、教室内での説明共有
- IPアドレス・同じネットワークの範囲・MACアドレス・機器番号と差込口の対応表・ルータの道案内・DNSの登録内容・Webページ要求の読み方と確認手順
- `easy_Packet`の教材要素を統合した模擬ターミナル（ipconfig、arp、nslookup、ping、traceroute、HTTPS）
- 「目的を読む→予想する→確認コマンドを実行する→結果を見る→自分の言葉で説明する」を繰り返すコマンド実験
...
- 題材のURLには文部科学省の公開ページを使用しますが、表示するIPアドレスや応答は学習モデルの値であり、実際の文部科学省サイトへは接続しません。
